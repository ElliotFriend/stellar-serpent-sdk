...
| File | Change |
|---|---|
| `src/serpent/types/buffers.py:180-237` | `Bytes65`; `_BYTES_N_CACHE` gains 65; `bytes_n`'s stale "sub-plan C" sentence corrected (E4); `String` accepts and orders `surrogateescape` text. |
...
| `src/serpent/env.py:216-242, 1346-1560` | Four `DEFAULT_*` constants; `Ledger`'s three new readers; `Env.__init__`'s four keywords; `Env.current_contract_address()`; `Crypto`/`Prng`/`Logs` and their accessors; `CryptoTrap`/`PrngTrap`; the frame PRNG reset in `_invocation`; `recorded_logs`. |
...
| `src/serpent/compiler/recognize.py` | 15 `RECOGNIZED` rows; `_recognize_ledger_method` grows three arms and loses `_LEDGER_FUTURE_METHODS`; `_recognize_crypto_method`, `_recognize_prng_method`, `_recognize_logs_method`, `_recognize_current_contract_address`; `_CORE_ENV_SURFACES` grows; `KNOWN_FUTURE_ENV_NAMES` shrinks to three; the two conversion rows; `_HELP` for SPT1040. |
| `src/serpent/compiler/codes.py` | The FOUR sanctioned edits (Task 4 ONLY). |
| `src/serpent/compiler/frontend.py:197-213, 904-946` | `log_from_linear_memory` joins `_LINEAR_MEMORY_HOST_FNS`; `_needs_memory` is unchanged in shape (the `used & _LINEAR_MEMORY_HOST_FNS` row answers it) and gains a test. |
| `src/serpent/emitter/lower.py:1348-1366` | The `val_typed_ret is False` boxing branch; `_lower_log`. |
| `tests/unit/test_recognize_env.py:273-306` | `_DOSSIER_C4_INVENTORY` grows to the enumerated 25 names. |
...
| `tests/harness/hostfns.py` | 15 new bindings, four new attributes, the log reader and `logs` list. |
...
| `tests/unit/test_examples.py:135-150` | The prose naming the protocol-22 examples gains the raffle. |
| `tests/unit/test_docs_site.py:70-81` | The strict build asserts the raffle's page too. |
| `tests/unit/test_no_stale_promises.py` | A FIFTH net for A's own forward references (`sub-plan A`, `\bA's\b` is NOT used -- see Task 12). |
...
| 11 | **Sonnet** | **Opus** | Mechanical once the surfaces exist (D11's pattern), but it produces a WAT golden and a real-host leg, so the REVIEW is seated up. **Plan-author refinement to the ruling's letter; ratify or overturn at plan review.** |
...
def test_a_degenerate_range_answers_without_drawing() -> None:
    """`lo == hi`: `range == 1`, `z == 0`, and the first draw always lands."""
...
def u64_in_inclusive_range(rng: ChaCha20Rng, lo: int, hi: int) -> int:
    ...
    """
    if lo > hi:
        raise ValueError(f"prng_u64_in_inclusive_range needs lo <= hi, got lo={lo} hi={hi}")
...
def test_ed25519_accepts_a_non_canonical_y_and_reduces_it() -> None:
    ...
    for small_y in range(19):  # the only y values for which y + p fits in 255 bits
        ...
        non_canonical = _crypto.edwards_decompress((small_y + _crypto.ED25519_P).to_bytes(32, "little"))
...
Performance, measured 2026-09-11 on CPython 3.11: keccak 0.33 ms on a short
input, ed25519 3.4 ms, secp256k1 recovery 11 ms, P-256 verify ~6 ms. Modular
...
def edwards_decompress(encoded: bytes) -> EdwardsPoint | None:
    ...
    never rejected. `None` means the `y` names no curve point, or the sign bit
    is set on the point with `x == 0` (dalek's one explicit refusal).
    """
    if len(encoded) != 32:
        return None
...
def ed25519_verify(public_key: bytes, message: bytes, signature: bytes) -> bool:
    ...
    return edwards_equal(expected, r_point)
...
def secp256k1_recover(digest: bytes, signature: bytes, recovery_id: int) -> bytes | None:
    ...
    The checks run in the HOST'S ORDER (`host.rs:3051-3062`): the signature
...
Run: `uv run --no-sync pytest -q tests/unit/test_crypto_primitives.py`
Expected: `20 passed` (the million-byte keccak vectors make this file take about 5 seconds; that is the price of F2's evidence and is stated here so nobody "optimises" it away).
...
SERPENT_REQUIRE_REAL_HOST=1 uv run --no-sync pytest -q
```
Expected: `4826 passed / 7 skipped` (4792 + 34 new). Report the actual numbers.
...
The four gates. Expected: `4836 passed / 7 skipped`. Report the actual numbers and any test elsewhere that moved (a `String`-strictness test asserting the OLD refusal is a real finding: report it, and change it only to the new documented behaviour).
...
**Files:**
- Modify: `src/serpent/env.py:216-232` (the module `__all__`), `:640-700` (beside `StorageTrap`), `:1386-1460` (the three new classes), `:1481-1620` (`Env`'s accessors, `_invocation`, `recorded_logs`)
...
Add `"CryptoTrap"` and `"PrngTrap"` to `env.py`'s module `__all__` (env.py:216-227), keeping it sorted. They are deliberately NOT in `serpent.__all__`: like `StorageTrap`, they are caught from `serpent.env`, which is where the tier-1 model lives, and `serpent.__all__` is the AUTHORING surface a compiled contract resolves names against.
...
class Prng:
    ...
    **Lifetime.** The host gives every FRAME its own PRNG and destroys it with
    the frame, so a reseed does not survive an invocation. This model does the
    same: `Env._invocation` resets the generator on frame entry. Note for
    M2-B, which owns tier-1 frame rollback: this is the only frame-scoped
    state M2-A adds, and the rollback design has to fold it in.
    """
...
In `_invocation`, immediately after `token = _frame.enter(self)`:

```python
        # The host gives every FRAME its own PRNG and destroys it with the
        # frame (K13.2), so a reseed cannot leak out of an invocation. Reset on
        # ENTRY rather than on exit: entry is the point both `frame()` and
        # `deploy` pass through, and a reset on exit would leave the state of a
        # frame that raised visible to the next one.
        self._prng_state = _prng.from_prng_seed(UNSEEDED_PRNG_SEED)
```
...
Run: `uv run --no-sync pytest -q tests/unit/test_env_crypto_prng_logs.py` -> `18 passed`.

- [ ] **Step 5: Gates and commit**

The four gates. Expected: `4854 passed / 7 skipped`.
...
Replace `_recognize_ledger_method`'s tail so the three new methods are lowered and `_LEDGER_FUTURE_METHODS` disappears (the helper below removes the repetition the three-arm shape would otherwise add):
...
def _leaf_args(
    ...
    for param, declared in zip(params, slots, strict=True):
        ...
        if not _assignable(value.ty, declared):
            _error(ctx, "SPT3018", loc, f"`{param}` is {value.ty.render()}, not {declared.render()}")
            return None
...
def _recognize_logs_method(node: ast.Call, ctx: FuncCtx, method: str) -> IRExpr:
    ...
    for argument in node.args[1:]:
        if isinstance(argument, ast.Starred):
            _error(ctx, "SPT1030", loc, "a starred argument is not supported")
            return _invalid(loc)
...
Confirm `SPT1030`'s intent covers a starred call argument before using it above; if it does not, use `_FALLBACK_CODE` and report the substitution (do NOT add a code).
...
and its SPT1033 detail becomes `f"`env.{name}` is recognized but not lowerable yet; it lands in M2-B"`.
...
**Report:** the enumerated inventory as it was transcribed (so the reviewer checks the ENUMERATION, F11); the two reductions' outcomes and the finding that SPT1034's "receiver not an owned local" half has no instance; the `git diff --stat docs/subset.md` and a summary of every changed section; the byte-freeze output quoted; the four gate outputs; the counts; whether SPT1030 was the right code for a starred log argument; any BLOCKED condition.
...
(`start`, `items_and_ctx`, `host`, `const` come from `tests/unit/test_emitter_lower_objects.py`; add `items_and_ctx` there if only `items` exists -- a two-line sibling returning `(probe.finish(), ctx)`. `store.last_prng_draw` and `store.u64_of` land in Task 8; until then, write the first test to assert only the TAG and add the value assertion in Task 8, and say so in the report.)
...
**Files:**
- Modify: `src/serpent/compiler/frontend.py:197-213` (`_LINEAR_MEMORY_HOST_FNS`); `src/serpent/emitter/lower.py` (`_LOG_FN`, `_lower_log`, the arm in `_lower_host_call`)
- Create: `tests/unit/test_emitter_log.py`
- Test: the new file, plus `tests/unit/test_frontend.py`, `test_emitter_end_to_end.py`

**Interfaces:**
- Consumes: `ctx.memory.intern(blob, align=...)` and `ctx.memory.scratch(nbytes)` (layout.py:64-86); `_store_val(fn, ctx, address, e)` (lower.py:1557-1569); `val.pack_u32val`; `module.check_linear_memory_abi`; `frontend._needs_memory`'s `used & _LINEAR_MEMORY_HOST_FNS` row.
- Produces: `lower._LOG_FN`, `lower._lower_log(fn, ctx, e)`.
...
```python
    ...
    def get_ledger_network_id(self) -> int:
        ...
        return self._new(val.TAG_BYTES_OBJECT, Bytes(self.network_id))
    ...
    def compute_hash_sha256(self, x: int) -> int:
        ...
        return self._new(val.TAG_BYTES_OBJECT, Bytes(_crypto.sha256(self.bytes_of(x))))

    def compute_hash_keccak256(self, x: int) -> int:
        """`c.1`, same delegation."""
        self._log("compute_hash_keccak256", x)
        return self._new(val.TAG_BYTES_OBJECT, Bytes(_crypto.keccak256(self.bytes_of(x))))
    ...
    def recover_key_ecdsa_secp256k1(self, msg_digest: int, signature: int,
        ...
        return self._new(val.TAG_BYTES_OBJECT, Bytes(recovered))
...
Add `u64_of(self, word: int) -> int` to `ObjectStore` if it does not exist (it needs to unbox a `U64Small`/`U64Object` for Task 6's test); if `val` already has a helper, use it and say so.
...
- Modify: `tests/semantics/env_scenarios.py`, `tests/semantics/host_facts.py:161-244`, `tests/fixtures/host_facts.py`, `tests/unit/test_harness_hostfns.py:1006` (`_FIXTURES`), `.github/workflows/ci.yml:140-148`
...
`tests/fixtures/prng_surface.py` is a small contract with one method per row (`draw_bytes`, `draw_full_range`, `draw_range`, `draw_shuffle`, `draw_twice`), each one seeding first from a `Bytes` parameter.

`tests/unit/test_prng_differential.py` replays every row at tier 1 (`deploy` + `env.frame()`) and at tier 2a (the mini host), asserting both equal `expect`. `tests/real_host/test_prng_real.py` replays them against `RealEnv`, plus:
...
**Report:** the measured shuffle permutation and the measured unseeded draws, quoted from the real-host run that produced them; confirmation that the three `_NO_MAXIMUM` rows are still `Unmodelled` and still the only three; whether `bytes(range(256))` survived `_scval` (E7's UNVERIFIED question -- answer it); the new `REAL_HOST_FLOOR`; the gate outputs; the counts.

---

### Task 11: the ninth example -- `examples/raffle.py` and its nine inventory joins
...
class Raffle:
    def __init__(self, env: Env, admin: Address) -> None:
        ...
        env.storage().instance().set(DataKey.Entrants, Vec(Address, []))
    ...
    def enter(self, env: Env, who: Address) -> U32:
        ...
        entrants = env.storage().instance().get(DataKey.Entrants, Vec[Address])
        entrants.push_back(who)
        env.storage().instance().set(DataKey.Entrants, entrants)
        Entered(who=who, at_ledger=env.ledger().sequence()).publish(env)
        return U32(len(entrants))
    ...
    def reveal(self, env: Env) -> Address:
        ...
        entrants = env.storage().instance().get(DataKey.Entrants, Vec[Address])
        index = env.prng().u64_in_range(U64(0), U64(len(entrants)) - U64(1))
        winner = entrants.get(U32(index))
        env.storage().instance().set(DataKey.Winner, winner)
        env.logs().add("raffle winner drawn", winner)
        Drawn(winner=winner, entrants=U32(len(entrants))).publish(env)
        return winner

    def shuffled_entrants(self, env: Env) -> Vec[Address]:
        """The entrant list in a pseudo-random order, for display.

        The shuffle is drawn from the CURRENT frame's PRNG, which is not
        reseeded here, so the order is not reproducible off chain and nothing
        may depend on it. It exists because a draw order is the natural way to
        show a raffle's field, and because the shuffle host function needs a
        surface at all.
        """
        entrants = env.storage().instance().get(DataKey.Entrants, Vec[Address])
        return env.prng().shuffle(entrants)
...
`_require_admin` is a private method, which E8's internal-call machinery supports; if the frontend refuses a leading-underscore method on a `@contract` class, promote it to a module-level helper taking `env` (the `guestbook.py` shape) and say so in the report. Likewise, verify `Vec[Address]` is the accepted spelling for the `get` type argument in this codebase (the guestbook uses `Message` and `U32`); if a subscripted generic is refused there, use whatever `examples/allowance_token.py` uses for a container-typed read.

**This contract must COMPILE before anything else in this task runs.** `uv run --no-sync python -c "from serpent.compiler import compile_module; compile_module(open('examples/raffle.py').read(), 'examples/raffle.py')"`. Every rejection is a real finding about the M2-A surface, to be reported -- not routed around by simplifying the example until it passes.

- [ ] **Step 2: the nine inventory joins (C26, F10)**
...
three ledger accessors, and current_contract_address -- and joins all nine
...
#
# The fourth net's shape, with A's letter. TWO needles and no more:
#
...
3. **Decisions the rulings did not settle**, for controller ratification at the final review: the `String` `surrogateescape` widening, the `s.to_bytes()`/`b.to_string()` names, the SPT1041 non-addition, the SPT1034 widening's single applicable shape, `Task 11`'s seating refinement, and any the execution added.
4. **UNVERIFIED claims carried out**, each with what would settle it: whether a `Crypto` error is observable as a recoverable value across a `try_call` boundary (M2-B's first probe, E3); the shadow-budget cost of a log at chain-realistic scale (F8); the shuffle arm beyond the one measured row (F5).
5. **Obligations carried OUT of M2-A**, with an owner: the TTL clamp/trap/floors and the three `Unmodelled` rows (M2-D); tier-1 frame rollback, into which A's frame-local PRNG reset must be folded (M2-B); `env.py`'s package promotion, now ~2,150 lines (M2-E); the ZK crypto families (M3).
...
| **D-7** evidence at the real host | T10 (all five steps) |
...
| **E13** `examples/raffle.py`; the four unused functions via a fixture contract | T11; T10 Step 1 (`crypto_surface.py`) |
...
| **F5** a PRNG whose stream is wrong | T1 Steps 1-2 (the three sdk vectors, the geometry, both sampler arms); T10 Step 2 (the three-tier corpus, incl. the ONLY shuffle evidence that will ever exist) |
...
| **F10** an inventory omission in the ninth example | T11 Step 2 (all nine, with the three untested ones called out) |
...
| **C24** the promise nets | T12 Step 2 (a fifth net, with the `\bA's\b` omission recorded) |
| **C25** `docs/subset.md` | T4 Step 7, re-checked in T12 Step 3 |
| **C26** the nine example inventories | T11 Step 2 |
...
Listed for ratification at plan review. Each is reversible at the cost stated.

1. **`String` is widened to carry arbitrary chain bytes through `surrogateescape`** (T2 Step 2). Ruling E7 adopts a `surrogateescape` round trip, but `String.__init__` currently REFUSES text that is not strictly UTF-8 encodable -- so the adopted behaviour is not implementable without this change. The widening makes the model accept exactly what the host accepts, which is the correct direction for a value constructor, but it is a value-layer behaviour change with a blast radius (`_order_key`, `_scval`, the mini host's two text readers). **Reversal cost: low but public.**
2. **The conversion surfaces are `s.to_bytes()` and `b.to_string()`** (T4 Step 3), value methods in the container family. E3 enumerated every other authoring name and stopped short of these. **Reversal cost: medium (a documented authoring name).**
3. **SPT1041 is not added** (Global Constraints, T4 Step 2), because an untyped log value already draws SPT3008. **Reversal cost: zero now, HIGH once published.**
4. **The SPT1034 widening names only the discarded-result shape** (T4 Step 2), because a functional shuffle has no aliasing hazard and the ruling's "receiver is not an owned local" half has no instance. **Reversal cost: zero (wording).**
5. **The crypto argument slots are typed `Ty.Bytes`, with a targeted fixed-length check** rather than `Ty.BytesN(n)` (T4 Step 4), because `_assignable` does not widen in that direction and a `BytesN` slot would refuse values the host accepts. **Reversal cost: zero.**
6. **Task 11 is Sonnet implementer + Opus review** rather than Sonnet on both sides (Model seating), because the raffle produces a WAT golden and a real-host leg. **Reversal cost: zero.**
7. **The fifth promise net uses `M2-A` as a needle and deliberately omits `\bA's\b`** (T12 Step 2). **Reversal cost: zero.**
8. **The shuffle vector's and the unseeded draws' expected values are MEASURED once on the real host and then pinned**, under a stated procedure that forbids fitting tier 1 to the row (T10 Step 2). **Reversal cost: zero.**

### Two decomposition defects this plan found (R7 asks the review to look)
...
- **Executed sanity checks** (run from the scratch directory against this file's OWN code blocks, extracted verbatim, 2026-09-11, `.venv/bin/python` 3.11.7):
  ...
  - Both modules pass `mypy --strict` and `ruff check --line-length 100` with no findings, and `ruff format --diff` is clean.
